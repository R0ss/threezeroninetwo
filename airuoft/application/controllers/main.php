<?php //functions that activate on click
...
class Main extends CI_Controller {
	...
	function departureData()
	{
		...
		$flights = $this->flight_model->get_departures();
		...
		if ($flights->num_rows() > 0){
		   ...
		   foreach ($flights->result() as $row){
				...
				$table[] = array($_POST['campus']->from,$row->to,$row->time,$_POST['departureDate']->date,$row->available);
		   } 
